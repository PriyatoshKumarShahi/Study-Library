...
import { BookOpen, Filter, Download, Search, Calendar, GraduationCap, FileText, Star, Eye } from 'lucide-react';
    ...
    try {
      ...
      const mockNotes = [
        ...
        },
        {
          ...
          fileUrl: '#',
          description: 'Logic gates, flip-flops, and digital circuits',
          downloads: 156,
          rating: 4.7
        }
      ];
  ...
  const applyFilters = () => {
    ...
    if (filters.search) {
      filtered = filtered.filter(note =>
        ...
        note.uploadedBy.toLowerCase().includes(filters.search.toLowerCase())
      );
    }
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredNotes.map((note) => (
            <div key={note.id} className="group bg-gray-800/80 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 hover:border-blue-500/50 transition-all duration-300 transform hover:-translate-y-2 shadow-2xl hover:shadow-blue-500/10">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-sm text-gray-300">{note.rating}</span>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-4">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-300">{note.year} • {note.branch}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Calendar className="w-4 h-4 text-green-400" />
                  <span className="text-gray-300">By {note.uploadedBy}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                  <Download className="w-4 h-4 text-purple-400" />
                  <span className="text-gray-300">{note.downloads} downloads</span>
                </div>
              </div>

              <div className="flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 px-4 py-2 rounded-lg transition-all duration-200 transform hover:scale-105 group">
