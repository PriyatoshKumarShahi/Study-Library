...
export default function FacultyDashboard() {
  ...
  const handleSubmit = async (e) => {
    ...
    // --- Normalize digit-only fields before uploading ---
    const normalizedForm = {
      ...form,
      assignmentNo: form.assignmentNo.replace(/\D/g, ""), // only digits
      year: form.year.replace(/\D/g, ""), // only digits
    };
    ...
    if (!normalizedForm.facultyName && user?.name)
      data.set("facultyName", user.name);
    ...
    <div className="min-h-screen bg-gray-900 text-white relative overflow-hidden">
      ...
      {uploading && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
          <Loader message="Uploading assignment..." />
        </div>
      )}
